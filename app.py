...
if not API_KEY:
    st.error("Gemini API key not set! Please set GOOGLE_API_KEY in Streamlit Secrets.")
else:
    ...
    def gemini_generate_stickers(prompt, num_images=10):
        try:
            ...
            sticker_prompt = f"Generate a cute, colorful cartoon sticker with transparent background, style: sticker, theme: {prompt}. No text, no watermark."
            images = []
