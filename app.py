...
def gemini_generate_stickers(prompt, num_images=10):
    try:
        model = genai.GenerativeModel('gemini-1.5-pro-latest')
        sticker_prompt = f"Generate a cute, colorful cartoon sticker with transparent background, style: sticker, theme: {prompt}. No text."
        images = []
        for i in range(num_images):
            response = model.generate_content(sticker_prompt, stream=False)
            # Gemini may return base64 images in the parts
            # Fallback to URL if available, else skip
            img_data = None
                ...
                for cand in response.candidates:
                    for part in cand.content.parts:
                        if hasattr(part, 'inline_data'):
                            ...
                            img_data = part.file_data.data
                        elif hasattr(part, 'url'):
                            img_data = part.url  # fallback, but prefer base64
                        if img_data:
                            break
                    if img_data:
                        break
            ...
            if isinstance(img_data, str) and img_data.startswith("http"):
                ...
                # Assume base64 (preferred)
                images.append(img_data)
        return images
    except Exception as e:
        st.warning(f"Gemini sticker generation failed: {e}")
        return []
...
def export_pdf(entry_date, entry_text, mood_emoji, images, stickers):
    ...
    for sidx, sticker in enumerate(stickers):
        ...
        # sticker is base64, decode and save
        import base64
        imgdata = base64.b64decode(sticker)
...
with tab1:
    ...
    if st.button("Generate Stickers"):
        if not sticker_prompt.strip():
            ...
            sticker_images = gemini_generate_stickers(sticker_prompt)
        ...
        for idx, img in enumerate(sticker_images):
            with cols[idx % 5]:
                if isinstance(img, str) and img.startswith("http"):
                    ...
                    st.image(Image.open(io.BytesIO(base64.b64decode(img))), width=80)
    ...
    if st.button("Fetch Entry"):
        ...
        if entry_text:
            ...
            if stickers:
                ...
                for sidx, sticker in enumerate(stickers):
                    with stx[sidx % 5]:
                        if isinstance(sticker, str) and sticker.startswith("http"):
                            st.image(sticker, width=60)
                        else:
                            st.image(Image.open(io.BytesIO(base64.b64decode(sticker))), width=60)
            ...
            with st.expander(f"{e_date} | Mood: {mood}"):
                    ...
                    for sidx, sticker in enumerate(stickers):
                        with stx[sidx % 5]:
                            if isinstance(sticker, str) and sticker.startswith("http"):
                                ...
                                st.image(Image.open(io.BytesIO(base64.b64decode(sticker))), width=50)
                ...
                with col2:
                    if st.button("Edit", key=f"edit_{entry_id}"):
                        ...
                        # sticker edit not implemented in edit mode
